"""Scryfall API data models (cards, sets, rulings, bulk data)."""
...
class Card(BaseModel):
    ...
    Notes
    -----
    Fields that Scryfall omits for some card variants (e.g. reversible
    cards lack a top-level ``oracle_id``) are optional with safe defaults,
    so ONE unusual card cannot fail validation for a whole search page.
    """
    ...
    legalities: Legalities = Field(default_factory=Legalities)
    ...
    set_name: str
    set_type: str
    set_uri: HttpUrl | None = None
    set_search_uri: HttpUrl | None = None
    scryfall_set_uri: HttpUrl | None = None
    rulings_uri: HttpUrl | None = None
    prints_search_uri: HttpUrl | None = None
    ...
    related_uris: RelatedUris = Field(default_factory=RelatedUris)
