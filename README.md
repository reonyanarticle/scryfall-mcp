...
- **Remote MCP対応**: AWS Lambda/Streamable HTTP transportによるクラウドデプロイ
- **認証システム**: OAuth 2.1 + JWT認証、Email-based認証（開発用）
- **セキュリティ**: bcrypt暗号化、GDPR/CCPA準拠のPIIマスキング、API Gateway throttling
...
│   │   └── sets.py       # セット情報取得・キャッシング
│   ├── auth/             # 認証・認可システム (NEW)
│   │   ├── middleware.py # JWT/Email認証ミドルウェア
│   │   ├── oauth.py      # OAuth 2.1クライアント
│   │   └── email.py      # Email-based認証 (bcrypt)
│   ├── cache/            # 2層キャッシュシステム
│   │   ├── backends.py   # Memory/Redis/Composite
│   │   └── manager.py    # キャッシュマネージャー
│   ├── i18n/             # 国際化・多言語対応
│   │   ├── constants.py  # 静的語彙データ
│   │   ├── locales.py    # ロケール管理
│   │   └── mappings/     # 言語マッピング
│   ├── search/           # 検索パイプライン
│   │   ├── parser.py     # 自然言語解析
...
│   ├── settings.py       # 実行時設定管理
│   └── models.py         # リクエストモデル＋再エクスポートfaçade（実体は各層のmodels.py）
├── deploy/               # Remote MCPデプロイ設定 (NEW)
│   └── aws/              # AWS Lambda デプロイ
│       ├── serverless.yml # Serverless Framework設定
│       └── lambda_handler.py # Lambda エントリーポイント
├── tests/                # テストスイート
├── docs/                 # ドキュメント
└── CLAUDE.md             # AI開発者向けガイド（要約＋ポインタ）
```
...
詳細な実装については [AI開発者ガイド](CLAUDE.md) を参照してください。
