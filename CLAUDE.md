...
Remote MCP (HTTP/Streamable HTTP transport)により、Scryfall MCP ServerをHTTPエンドポイントとして公開し、Claude.aiなど外部クライアントから利用可能にします。**コスト最適化版**では、ElastiCache/VPCを削除し、メモリキャッシュのみを使用する軽量構成で**月額$0-2**を実現します。
...
**主要な設定項目**:
- **runtime**: Python 3.11, ARM64 (Graviton2、20%コスト削減)
- **メモリ**: 256MB（512MBから削減）
- **タイムアウト**: 30秒
...
```python
# Global cache for server instance (cold start optimization)
_server: ScryfallMCPServer | None = None

def get_server() -> ScryfallMCPServer:
    """Get or create server instance (lazy initialization)."""
    global _server
    if _server is None:
...
**将来実装予定**: `scripts/manage_secrets.py` (Typer CLI)により、上記コマンドをインタラクティブに実行可能にする。
